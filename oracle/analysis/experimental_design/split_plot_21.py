...
class SplitPlot21Analyzer:
    def __init__(self, df, main_a_col, main_b_col, sub_c_col, rep_col, resp_col):
        self.df = df
        self.a_col = main_a_col
        self.b_col = main_b_col
    ...
    def run_anova(self):
        ...
        # 5. Error (a) - Main Plot Error
        # Formula: Sum(RiAjBk)^2/c - SS_R - SS_A - SS_B - SS_AxB - CF
        # Correction: The formula says SS_Error(a) = Sum(RiAjBk)^2/c ... 
        # Actually, standard split plot error(a) is usually interaction of Rep * MainPlots.
        # Here MainPlots is combination A*B. So Error(a) is Rep * (A*B).
        # SS_RepxAxB = get_ss([rep, a, b], c). 
        # SS_Error_a = SS_RepxAxB - SS_R - SS_A - SS_B - SS_AxB.
        SS_RepAxB_Total = get_ss([self.r_col, self.a_col, self.b_col], nC)
        SS_Err_A = SS_RepAxB_Total - SS_R - SS_A - SS_B - SS_AxB
        ...
        # SS_Total - (all above)
        SS_Err_B = SS_Total - SS_R - SS_A - SS_B - SS_AxB - SS_Err_A - SS_C - SS_AxC - SS_BxC - SS_AxBxC
        ...
        df_b = nB - 1
        df_axb = df_a * df_b
        df_err_a = (nR - 1) * (nA * nB - 1)
        
        df_c = nC - 1
        df_axc = df_a * df_c
        df_bxc = df_b * df_c
        df_axbxc = df_a * df_b * df_c
        df_err_b = nA * nB * (nR - 1) * (nC - 1)
        ...
        # F Tests
        # Main Plot Effects -> Err A
        F_R = MS_R / MS_Err_A
        F_A = MS_A / MS_Err_A
        F_B = MS_B / MS_Err_A
        F_AxB = MS_AxB / MS_Err_A
        ...
        P_B = get_p(F_B, df_b, df_err_a)
        P_AxB = get_p(F_AxB, df_axb, df_err_a)
        ...
        self.anova_table = {
            ...
            "Factor B (Main)": s(df_b, SS_B, MS_B, F_B, P_B),
    ...
    def run_post_hoc(self, method, alpha):
        ...
        sem_b = calc_sem(MS_Ea, self.n_a * self.n_c * self.n_r)
        sem_axb = calc_sem(MS_Ea, self.n_c * self.n_r)
        ...
        stats_map = {
            ...
            "Factor B": {"SEm": sem_b, "SEd": sem_b*np.sqrt(2), "CD": calc_cd(sem_b, DF_Ea, self.n_b)},
            "Interaction A x B": {"SEm": sem_axb, "SEd": sem_axb*np.sqrt(2), "CD": calc_cd(sem_axb, DF_Ea, self.n_a*self.n_b)},
        ...
        def proc_eff(name, cols, sem_key, df_err, n_lvls):
            ...
            # Re-map key name for P check if mismatch (e.g. Factor A (Main) vs Factor A)
            # My stats_map uses clean names. ANOVA uses descriptive.
            # Let's trust stats_map keys for the display.
            
            let = {k: "ns" for k in m.index}
            # Need strict P check? Yes, assume always output table but "ns" if P > alpha
            if p < alpha:
        ...
        proc_eff("Factor B (Main)", self.b_col, "Factor B", DF_Ea, self.n_b)
        proc_eff("Interaction A x B", [self.a_col, self.b_col], "Interaction A x B", DF_Ea, self.n_a*self.n_b)
    ...
    def create_report(self):
        doc = Document()
        doc.add_heading('Split Plot (2,1) Analysis Report', 0)
        ...
        keys = ["Replication", "Factor A (Main)", "Factor B (Main)", "Interaction A x B", "Error (a)",
                "Factor C (Sub)", "Interaction A x C", "Interaction B x C", "Interaction A x B x C", "Error (b)", "Total"]
        ...
        stats_keys = ["Factor A", "Factor B", "Interaction A x B", "Factor C", "Interaction A x C", "Interaction B x C", "Interaction A x B x C"]
        for k in stats_keys:
             s = self.stats[k]
